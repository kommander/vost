...
var Vost = module.exports = function(settings){
  ...
  for(var hostNum = 0; hostNum < this.settings.hosts.length; hostNum++){
    ...
    if(typeof host.target === 'string'){
      // Single target
      targets.push(this._createHostTarget(host));
    ...
    // Add 503 message for all targets
    targets.message503 = host.message503 || null;
 ...
 */
Vost.prototype._createHostTarget = function(host) {
  if(typeof host.target !== 'string'){
  ...
  var targetArr = host.target.split(':');
  var targetObj = {
  ...
  if(hostArr != null){
    ...
    if(targets != null && targets.length > 0){
      ...
      if(selectedTarget !== null){
        ...
        this._replyHostDown(sock, targets.message503);
        return;
      }
    }
...
Vost.prototype._replyHostDown = function(clientSocket, message) {
  // TODO: Create response on startup and reuse

  // Check if host specific 503 Message is available
  var message503 = message != null 
    ? message
    : this.settings.message503
  ;
  ...
  if(typeof message503 == 'string'){
    options.body = message503;
  } else if(typeof message503 == 'object'){
    // Take state from settings
    if(message503.state)
      options.state = message503.state;

    // Take headers from settings
    if(message503.headers)
      options.headers = message503.headers;

    options.body = message503.body;
  }
...
Vost.prototype._failTarget = function(targetSocket) {
  ...
  if(targetSocket.targets.length > 1){
    ...
    if(selectedTarget !== null){
      ...
      this._replyHostDown(targetSocket.clientSocket, targetSocket.targets[targetSocket.selectedTarget].message503);
    }      
    return;
  }
